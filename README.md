...
**Redeploy QC + PCD (2026-09-25, `v1.2.0`):** stack di-rebuild di VM. **`/predict-hand` kini aktif di produksi** — `ml_service/pcd_core.py` masuk image (fix Dockerfile `COPY app.py features.py pcd_core.py`); `288.jpg` → anemia 0.383 · 8 kuku, `54.jpg` → normal 0.166 · 10 kuku. **QC ketajaman aktif** di `ml`: foto buram → `400 "Foto terlalu buram (skor ketajaman 3)…"`; backend meneruskan sebagai `400 {message:…}` (app menampilkan "ambil ulang", bukan fallback mock). Sharp close-up → `201 source:"ml"`. Fix ikutan: `Body` import di `screening.controller.ts` (TS2693 sempat gagal `nest build`), disk VM di-prune (cache/build) saat 100% penuh — data Postgres (volume `anemia_pgdata`) utuh. Terverifikasi live via curl dari host: health OK, container `api`/`ml`/`db` healthy.
