 ...
 *
 * - Schema `dw`: dim_waktu + dim_hasil + fact_skrining (KONSEP §LAPISAN 3).
 ...
 * - dim_pasien/dim_lokasi belum dibuat: data pasien & wilayah belum ada di
 *   skrining (mengalir nanti dari fitur akun pengguna + konteks fuzzy).
 */
@Injectable()
...
BEGIN
  ...
  INSERT INTO dw.fact_skrining (skrining_id, dim_waktu_id, dim_hasil_id, confidence, hb_estimate)
  VALUES (NEW.id, w_id, h_id, NEW.confidence, NEW."hbEstimateGdl")
...
AFTER INSERT OR UPDATE OF indication, status, source, confidence, "hbEstimateGdl"
ON public.screening
